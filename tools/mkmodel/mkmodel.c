...
{
    memset(&all_textures, 0, sizeof(all_textures));
}

void free_texture_table()
{
    for (uint32_t i = 0; i < all_textures.num; i++) {
        free((char*)all_textures.paths[i]);
    }
}
...
{
    for (uint32_t i = 0; i < all_textures.num; i++) {
        if (strcmp(all_textures.paths[i], path) == 0) {
            printf("mapped path '%s' at index %u\n", path, i);
            return i;
        }
    }
    ...
    strncpy(new, path, size); // TODO: use strdup?

    uint32_t idx = all_textures.num++;
    all_textures.paths[idx] = new;
    printf("added path '%s' at index %u as '%s'\n", path, idx, new);
    return idx;
}
...
void model64_write_textures(model64_data_t *model, FILE *out) {
    ...
    for (uint32_t i = 0; i < all_textures.num; i++) {
        ...
        fwrite(all_textures.paths[i], strlen(all_textures.paths[i])+1, 1, out);
    }
...
static bool check_image_uri_is_path(const char *uri) {
    // FIXME please put in proper validation
    if (strlen(uri) > 5) {
...
int convert_primitive(cgltf_primitive *in_primitive, primitive_t *out_primitive)
{
    ...
    out_primitive->texture_idx = INVALID_TEXTURE_INDEX;
    ...
    if (in_primitive->material) {
        ...
        if (texture && texture->image) {
            ...
            printf("found image URI '%s'\n", uri);
            if (!check_image_uri_is_path(uri)) {
                fprintf(stderr, "Error: Image URI %s wasn't a path.\n", uri);
                free(weight_attr.pointer);
                return 1;
            }

            out_primitive->texture_idx = get_texture_index(uri);
        }
    }
